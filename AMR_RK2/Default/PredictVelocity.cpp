    ...
    for (int lev = 0; lev <= finest_level; lev++)
    {
        ...
        for (MFIter mfi(xvel[lev]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                uk1(i, j, k) =  (Mu * diff - adv);
                u(i, j, k) = uo(i, j, k) + dt * uk1(i, j, k); });
        }
        ...
        for (MFIter mfi(yvel[lev]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                vk1(i, j, k) = (Mu * diff - adv);
                v(i, j, k) = vo(i, j, k) + dt * vk1(i, j, k); });
        }
    ...
    for (int lev = 0; lev <= finest_level; lev++)
    {
        ...
        for (MFIter mfi(xvel[lev]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                Real adv = dxinv[0] * (ue * ue - uw * uw) + dxinv[1] * (vn * un - vs * us);
                uk2(i, j, k) = (Mu * diff - adv);
                u(i, j, k) = uo(i, j, k) + dt/2.0 * (uk1(i, j, k) + uk2(i, j, k)); });
        }
        ...
        for (MFIter mfi(yvel[lev]); mfi.isValid(); ++mfi)
        {
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                Real adv = dxinv[0] * (ue * ve - uw * vw) + dxinv[1] * (vn * vn - vs * vs);
                vk2(i, j, k) = (Mu * diff - adv);
                v(i, j, k) = vo(i, j, k) + dt/2.0 * (vk1(i, j, k) + vk2(i, j, k)); });
        }
    }
}