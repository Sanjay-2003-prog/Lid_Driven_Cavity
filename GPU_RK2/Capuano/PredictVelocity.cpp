...
void NavierStokesSolver::predictVelocity_k1()
{
    ...
    for (int lev = 0; lev <= finest_level; lev++)
    {
        ...
        for (MFIter mfi(xvel[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            const auto &uk1 = xvel_k1[lev][mfi].array();
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                uk1(i, j, k) = (Mu_val * diff - adv);
                u(i, j, k) = uo(i, j, k) + dt_val * uk1(i, j, k); });
        }
        ...
        for (MFIter mfi(yvel[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            const auto &uo = xvel_old[lev][mfi].array();
            const auto &vo = yvel_old[lev][mfi].array();
            const auto &vk1 = yvel_k1[lev][mfi].array();
            ...
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                ...
                vk1(i, j, k) = (Mu_val * diff - adv);
                v(i, j, k) = vo(i, j, k) + dt_val * vk1(i, j, k); });
        }
        // Print() << "1" << std::endl;
...
void NavierStokesSolver::predictVelocity_k2()
{
    BL_PROFILE("Predict Velocity");
    ...
    for (int lev = 0; lev <= finest_level; lev++)
    {
        ...
        for (MFIter mfi(xvel[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            const auto &vo = yvel_old[lev][mfi].array();
            const auto &uk1 = xvel_k1[lev][mfi].array();
            const auto &uk2 = xvel_k2[lev][mfi].array();

            // Capture everything by value
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                        {
                Real ue = 0.5 * (u(i + 1, j, k) + u(i, j, k));
                Real uw = 0.5 * (u(i - 1, j, k) + u(i, j, k));
                Real un = 0.5 * (u(i, j + 1, k) + u(i, j, k));
                Real us = 0.5 * (u(i, j - 1, k) + u(i, j, k));
                Real vs = 0.5 * (v(i - 1, j, k) + v(i, j, k));
                Real vn = 0.5 * (v(i - 1, j + 1, k) + v(i, j + 1, k));

                Real diff = dx2inv * (u(i + 1, j, k) - 2.0 * u(i, j, k) + u(i - 1, j, k)) 
                          + dy2inv * (u(i, j + 1, k) - 2.0 * u(i, j, k) + u(i, j - 1, k));
                Real adv = dxinv[0] * (ue * ue - uw * uw) + dxinv[1] * (vn * un - vs * us);
                uk2(i, j, k) = (Mu_val * diff - adv);
                u(i, j, k) = uo(i, j, k) + dt_val/2.0 * (uk2(i, j, k)+uk1(i, j, k)); });
        }
        ...
        for (MFIter mfi(yvel[lev], TilingIfNotGPU()); mfi.isValid(); ++mfi)
        {
            ...
            const auto &vo = yvel_old[lev][mfi].array();
            const auto &vk1 = yvel_k1[lev][mfi].array();
            const auto &vk2 = yvel_k2[lev][mfi].array();

            // Capture everything by value
            ParallelFor(bx, [=] AMREX_GPU_DEVICE(int i, int j, int k)
                        {
                Real ue = 0.5 * (u(i + 1, j, k) + u(i+1, j-1, k));
                Real uw = 0.5 * (u(i, j, k) + u(i, j-1, k));
                Real vn = 0.5 * (v(i, j, k) + v(i, j+1, k));
                Real vs = 0.5 * (v(i, j - 1, k) + v(i, j, k));
                Real ve = 0.5 * (v(i + 1, j, k) + v(i, j, k));
                Real vw = 0.5 * (v(i - 1, j , k) + v(i, j, k));

                Real diff = dx2inv * (v(i + 1, j, k) - 2.0 * v(i, j, k) + v(i - 1, j, k)) 
                          + dy2inv * (v(i, j + 1, k) - 2.0 * v(i, j, k) + v(i, j - 1, k));
                Real adv = dxinv[0] * (ue * ve - uw * vw) + dxinv[1] * (vn * vn - vs * vs);
                vk2(i, j, k) = (Mu_val * diff - adv);
                v(i, j, k) = vo(i, j, k) + dt_val/2.0 * (vk2(i, j, k)+vk1(i, j, k)); });
        }
        // Print() << "1" << std::endl;
    }
}